...
- **Um arquivo SQLite com disco persistente** (`data/dev.sqlite3` hoje). Se o disco for apagado a cada deploy/reinício, **todos os dados dos clientes somem**. É a restrição mais importante para escolher hospedagem.
...
**Pré-requisito para essa opção**: migrar o banco de SQLite para PostgreSQL (driver `pg`, funções assíncronas, ajustar o SQL das 8 migrações). É um trabalho de uma etapa própria, ainda **não feito** — o código atual roda só com SQLite. Sem isso, o Render gratuito perde os dados a cada deploy.
...
| `DATABASE_FILE` | Se SQLite em VM | Caminho do arquivo no disco persistente |
