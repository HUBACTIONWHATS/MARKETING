...
- **Banco por variável de ambiente (Etapa 9)**: `src/db.ts` é a única porta de acesso ao banco (`db.get/all/run/exec/transaction`, tudo `async`). Sem `DATABASE_URL` → SQLite (dev/testes); com `DATABASE_URL` → PostgreSQL (piloto na Neon). Regras para qualquer código novo: SQL portável (placeholders `?`, `INSERT ... RETURNING id`, timestamps ISO gravados pela aplicação — nunca `datetime('now')`, `CAST(? AS INTEGER) IS NULL` para parâmetro nulo, `tx` dentro de `db.transaction`); **toda migração nova precisa existir nas duas pastas**, `migrations/sqlite/` e `migrations/postgres/`, com o mesmo nome de arquivo; `npm test` roda no SQLite e `npm run test:pg` roda a mesma suíte num PostgreSQL real local (`embedded-postgres`, dev-only) — rodar os dois antes de entregar.
